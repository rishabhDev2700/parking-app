...
def home(request):
    ...
    ).select_related('parking_space', 'parking_space__parking_lot', 'parking_space__parking_lot__location')

    available_lots = ParkingLot.objects.filter(
        is_active=True,
        parkingspace__is_active=True
    ).annotate(
...
def locations_lots_list(request,location_id):
    ...
    return render(request,"parking/location_list.html",{"location":location,"parkinglots":lots})
...
def parking_space_by_lot(request, lot_id):
    ...
    parking_spaces = ParkingSpace.objects.filter(parking_lot=parking_lot, is_active=True)
...
def booking_summary(request):
    ...
    start_time_aware = timezone.make_aware(start_time_naive, timezone.get_current_timezone())
    ...
    total_price = parking_space.parking_lot.hourly_rate * Decimal(duration_hours)
    return render(request,'parking/booking_summary.html',{"start_time":start_time_display,"end_time":end_time_display,"parking_space":parking_space,"total_price":total_price})
...
def create_booking(request):
    ...
    if request.method == "POST":
        ...
        try:
            start_time = timezone.make_aware(parse_datetime(start_time),timezone.get_current_timezone())
            end_time = timezone.make_aware(parse_datetime(end_time),timezone.get_current_timezone())
            ...
            if not parking_space.is_available(start_time, end_time):
                return JsonResponse({"error": "Parking space is not available"}, status=400)
            session = BookingService.create_booking(request,parking_space,start_time,end_time,vehicle_model,vehicle_number)
...
def stripe_webhook(request):
    ...
    elif event["type"] in ["payment_intent.payment_failed", "checkout.session.expired","charge.failed","checkout.session.async_payment_failed",]:
        if booking_id:
            try:
                booking = Booking.objects.get(id=booking_id)
                booking.status = "cancelled"
...
def cancel_booking(request, booking_id):
    ...
    with transaction.atomic():
        ...
        try:
            ...
            checkout_session = stripe.checkout.Session.retrieve(payment.stripe_charge_id)
...
def user_bookings(request):
    ...
    return render(request, 'parking/user_bookings.html', {
